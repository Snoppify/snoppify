...
var specialInverted = Object.keys(special).reduce(function(obj, key) {
    obj[special[key]] = key;
    return obj;
}, {});
...
function getIP(code) {
    ...
    var codeTrim = a.join(" ");
    console.log(codeTrim, specialInverted);
    if (specialInverted[codeTrim]) {
        return specialInverted[codeTrim];
    }
    var invalid = false;
    a = a.map((e, i) => {
        for (var p in prio[i]) {

<template>
  <span class="track-item">
    ...
    <span v-if="track.snoppify">
      <button v-if="track.snoppify.issuer.id !== user.id" class="snopp-btn snopp-btn--medium snopp-btn--vote" v-bind:class="{active:track.snoppify.votes.indexOf(username) != -1}" v-on:click="vote(track)">
...
export default {
  ...
  methods: {
    ...
    queueTrack(track) {
      Vue.set(track, "snoppify", { issuer: this.user });
